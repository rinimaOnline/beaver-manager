...
// 七牛云文件上传
export async function uploadFileToQiniu(file: File, path: string, argument?: any): Promise<UploadInfo> {
  try {
    // 获取token（带缓存）
...
// 上传文件并保存到数据库
export async function uploadAndSaveFile(file: File, path: string, argument?: any): Promise<{ fileId: string } & UploadInfo> {
  try {
    // 1. 上传到七牛云
    const uploadInfo = await uploadFileToQiniu(file, path, argument)
